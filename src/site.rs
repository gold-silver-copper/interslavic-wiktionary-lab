...
function rankSuggestions(rows,q){const key=isvFold(q),first=Array.from(key)[0]||'';return(rows||[]).filter(([k])=>Array.from(k)[0]===first).map(([k,l])=>[lev(k,key),l]).filter(([d])=>d<=2).sort((a,b)=>a[0]-b[0]||(a[1]<b[1]?-1:a[1]>b[1]?1:0)).slice(0,3).map(x=>x[1]);}
async function rawSuggest(base,q){const key=isvFold(q),first=Array.from(key)[0]||'',shard=fnv1a32(first)%__SUGGEST_SHARDS__;if(!suggestCache[shard])suggestCache[shard]=fetch(base+'api/suggest/'+shard+'.json').then(r=>r.ok?r.json():{rows:[]}).catch(()=>({rows:[]}));const j=await suggestCache[shard];return rankSuggestions(j.rows,q);}

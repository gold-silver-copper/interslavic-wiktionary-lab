...
contain only verification-grade official/grammar lemmas; generated proposals
are excluded because this compact format has no status field. Rows follow the
CLI contract: same first letter, edit distance ≤2, nearest first, lexical
tie-break, at most three. `api/suggest-selftest.json` is generated by Rust and
the browser must pass it before displaying suggestions.

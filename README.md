...
- `cargo run --release -- corpus-calibrate` fits the dedicated corpus coverage
  model on the fixed 75% train split and measures it only on the untouched 25%
  holdout. The raw languages/branches coverage value is a ranking feature, not
  a probability or the pipeline candidate score.
- `data/corpus-coverage-calibration.json` records the score/label/algorithm
  versions, exact input SHA-256 digests, reliability bins and holdout metrics
  (ECE 0.0106; Brier 0.1080). Labels are a POS-plus-gloss official-coverage
  proxy: a negative is not proof that a reconstruction is linguistically wrong.
- `data/novel-words.tsv` is regenerated by every `export` from that compatible
  calibrator. On the unfiltered holdout population, the p≥0.6 and p≥0.3
  operating points measure 69.6% precision / 25.0% coverage and 53.1% / 46.9%.
  These figures are diagnostics before dictionary filtering; they are not the
  precision of the resulting unmatched proposal list.
...
src/
  ...
  calibrate.rs     separate pipeline/corpus calibration schemas and freshness checks
  corpus_calibrate.rs fixed-split corpus evaluator, PAVA fitter and reports
  corpus_reference.rs shared POS-plus-semantic official-sense matcher
...
- **`text-check.html`** — paste Interslavic text; every token is verified
  ...
  of `check-text`);
- **`proposals.html`** ("Predloženja novyh slov") — a corpus-ranked
  proposal/review worklist; its score is an unconditional official-coverage
  proxy, not acceptance probability or measured precision for unmatched rows;

...
| recoverable from the cache (official root present under a gloss-matched lemma) | 51 (2.8% of root-absent) |
| — of which reachable by the conservative rule (root under an uncited language) | 0 |
| — unreachable: root only under an already-cited language (adding it would displace the dictionary's own citation) | 51 |
